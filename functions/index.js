...
exports.generateEmail = onCall({secrets: [openaiApiKey]}, async (request) => {
  ...
  const getUserDescriptor = (userProfile, isConstituent) => {
    ...
    const tone = profile.communicationTone && profile.communicationTone.primaryTone;
  ...
  const prompt = `
    ...
    **COMMUNICATION STYLE:**
    ...
    - Personality Elements: ${tone.personalityElements ? tone.personalityElements.join(", ") : "None specified"}
    ${tone.personalityElements && tone.personalityElements.includes("EMPATHETIC") ? "- Show understanding of different perspectives" : ""}
    ${tone.personalityElements && tone.personalityElements.includes("DATA_SAVVY") ? "- Incorporate compelling statistics naturally" : ""}
    ${tone.personalityElements && tone.personalityElements.includes("STORY_DRIVEN") ? "- Use vivid anecdotes to illustrate points" : ""}
    ${tone.personalityElements && tone.personalityElements.includes("COMMUNITY_VOICE") ? "- Reference broader community impact" : ""}
    ...
    **POLICY POSITIONS TO REFLECT:**
    ...
    ${profile.supportedRegulations && profile.supportedRegulations.length > 0 ?
    ...
    ${profile.opposedRegulations && profile.opposedRegulations.length > 0 ?
